...
No test suite exists yet. `typecheck` + `build` are the safety net.

## Architecture

### Three-process layout
- **Electron main** (`electron/main/*.ts`) — IPC handlers, `buildService` (runs make / pio), `flashService` (dfu-util / pio upload), `serialService` (node-serialport), `sdk` (first-run libDaisy+DaisySP clone+build).
...
Adding a new DSP node requires edits in **five** places:
1. `src/types/graph.ts` — add to the `NodeKind` union.
2. `src/nodes/defs.<category>.ts` — add a `NodeDefinition` (label, category, inputs, outputs, params). The final `NODE_DEFINITIONS` is spread from `CORE_DEFS` + all partials in `src/nodes/definitions.ts`. Partials exist so parallel work doesn't collide on one giant file.
...
4. `src/audio/worklets/registry.<category>.ts` — add to the matching registry partial. Final `WORKLET_REGISTRY` merges them in `src/audio/worklets/registry.ts`. URL resolution uses `new URL('./x.worklet.ts', import.meta.url)`.
...
`src/audio/AudioEngine.ts` implements an interface in `src/types/store.ts`. Visual nodes call `engine.tap(nodeId, onFrame, { wantFrequency? })` to fork a node's output through a hidden `AnalyserNode`. A single global `requestAnimationFrame` loop pulls `getFloatTimeDomainData` / `getFloatFrequencyData` into reusable typed arrays and dispatches to all active taps. The visual renderers (scope/VU/spectrum/OLED) consume these frames. To tap a node's INPUT (like OLED elements bound to input sockets), walk the graph to find the connection's source and tap THAT source node — see `tapInput()` in `OledNode.tsx`.

### Theme system (Signal Lab is the default; 2 more are reserved)
`src/theme/ThemeProvider.tsx` flattens a `Skin` + shared tokens into CSS custom properties on `:root`. **Every component reads from `--dp-*` vars; no hex in component code.** Adding a theme = a new `Skin` in `src/theme/themes/`. The three planned: Signal Lab (live), Studio Rack, CRT Patchbay.
