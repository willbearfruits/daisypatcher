...
- **Electron main** (`electron/main/*.ts`) — IPC handlers, `buildService` (runs make / pio), `flashService` (dfu-util / pio upload), `serialService` (node-serialport), `sdk` (first-run libDaisy+DaisySP clone+build), `appMenu` (the application menu — a thin dispatcher that sends one command string per click over `app:command`; the renderer's `useMenuCommands` maps it onto the same store/file calls the keybindings and command palette use, so the three entry points cannot disagree), `sampleService`, `assistantService`, `updater` (electron-updater against GitHub Releases; only initialized when `app.isPackaged` — it throws on unsigned dev runs — and inert until the repo is public with a release containing `latest.yml`).
...
- Roadmap: **`ROADMAP.md` at the repo root is the current plan** — where the project actually stands (what is verified on hardware vs merely compiled), the researched competitive position, phased work with Phase 0 (hardware truth) gating everything, the decided `.dpnode` custom-node design, and the LAMINA bridge. Read it before proposing feature work. `V0_5_PLAN.md` is the record of the v0.5 milestone; `V2_PLAN.md` is the long-horizon vision.
...
- **Driving the real Electron window from outside** (the way this session found the keybinding shadow, the topbar overlap and the camera-on-load bug): `DP_CDP_PORT=9333 npm run dev` opens a Chrome DevTools Protocol endpoint (dev only — the switch is behind `isDev && env`, absent from packaged builds). `window.__dp = { store, engine }` is exposed in dev only (`import.meta.env.DEV`, verified stripped from the production bundle). Together they let a script call `store.getState().addNode(...)`, read the DOM, and screenshot via `Page.captureScreenshot` — or `import -window` at the OS level to see native chrome like the menu bar, which CDP cannot capture.
- **Menu accelerators are labels, not bindings.** Every item's `click` refuses `triggeredByAccelerator`; the chord is handled by `useGlobalKeybindings`, which knows about focus and text fields. `Space` and `Delete` carry no accelerator at all — a registered one swallows the key in inputs before the renderer sees it. When you add a shortcut, add it in the keybinding hook, then show it in `appMenu.ts` and the Shortcuts sheet (`AppModals.tsx`).
- **Undo history and graph-driven edits.** A `preset_recall` node firing off a clock is not a user edit. Such recalls go through `recallPreset(id, { silent: true })` / `beginTransactionSilent()` so they apply without a history entry — otherwise a clocked recall evicts every real edit from undo within seconds. Anything else the *patch* does to *itself* at audio rate should use the same path.
- **`loadGraph` backfills component `config`** from `hardware/defaultConfig.ts` (dependency-free so `scripts/build-examples.mjs` can use it too). A `.dpatch` with `config: {}` self-heals on open. Explicit values always win.
- **Examples ship in the app.** `package.json` `extraResources` copies `examples/*.dpatch` to `resources/examples`; `examples:list` grants those paths so `fs:readFile` (which otherwise only admits dialog-picked paths) can open them. Reachable from the empty canvas, File → Open Example…, and the command palette. Regenerate with `npm run examples` after changing the builder — descriptions come from `graph.meta.description`.
